...
exports.createEmployer = (req, res) => {
    ...
    Employer.create(employer).then(data => {
        ...
        res.status(500).send({
            message: err.message || "Algum erro ocorreu ao tentar criar o Funcionário!"
        });
    });
};

exports.findAllEmployer = (req, res) => {
    const position = req.query.position; //revisar esta constante...
    var condition = position ? { position: { [Op.like]: `%${position}%` } } : null; // Para case insensitive, use o operador Op.iLike!

    Employer.findAll({ where: condition }).then(data => {
        res.send(data);
    }).catch(err => {
        res.status(500).send({
            message: err.message || "Algum erro ocorreu ao tentar pesquisar os Funcionários!"
        });
    });
};

exports.findOneEmployerId = (req, res) => {
    const id_employer = req.params.id_employer;
    ...
    Employer.findByPk(id_employer).then(data => {
        if (data) {
            ...
            res.status(404).send({
                message: `Não foi possível encontrar o Funcionário com o ID: ${id_employer}.`
            });
        };
    }).catch(err => {
        res.status(500).send({
            message: "Algum erro ocorreu ao tentar encontrar o Funcionário com o ID: " + id_employer
        });
    });
};
...
exports.updateEmployer = (req, res) => {
    ...
    Employer.update(req.body, {
        ...
        if (num == 1) {
            res.send({
                message: "O Funcionário foi atualizado!"
            });
        } else {
            res.send({
                message: `Não foi possivel atualizar o funcionário com o ID: ${id_employer}.`
            });
        }
    }).catch(err => {
        res.status(500).send({
            message: "Algum erro ocorreu ao tentar atualizar o funcionário com o ID: " + id_employer
        });
    });
};
...
exports.deleteEmployerId = (req, res) => {
    ...
    Employer.destroy({
        ...
        if (num == 1) {
            res.send({
                message: "O Funcionário foi apagado com sucesso!"
            });
        } else {
            res.send({
                message: `Não foi possivel apagar o Funcionário com o ID: ${id_employer}.`
            });
        }
    }).catch(err => {
        res.status(500).send({
            message: "Algum erro ocorreu ao tentar apagar o funcionário com o ID: " + id_employer
        });
    });
};
...
exports.deleteAllEmployer = (req, res) => {
    Employer.destroy({
        ...
        res.send({
            message: `${nums} funcionários foram apagados com sucesso!`
        });
    }).catch(err => {
        res.status(500).send({
            message: err.message || "Algum erro ocorreu ao tentar apagar todos os funcionários!"
        });
    });
};

exports.findAllEmployerWithPerson = (req, res) => { //Inner Join entre Employer e Person
    Employer.findAll({
        include: [{
            model: db.Person,
            as: 'person'
        ...
        res.status(500).send({
            message: err.message || "Algum erro ocorreu ao tentar buscar funcionário associados com usuários."
        });
    });
};
*/